...
const DEFAULT_CONFIG_PATH = resolve(homedir(), ".multiteachercodex", "config.json");
const ENV_KEYS = [
    "PORT",
    "REVIEW_TIMEOUT_MS",
    "MAX_REVIEWERS",
...
async function readSavedConfig(path) {
    try {
...
async function writeSavedConfig(path, config) {
...
function applySavedConfig(config, overwrite = false) {
...
function runtimeDependenciesAvailable() {
...
function ensureRuntimeDependencies() {
    ...
    const result = spawnSync(npmCommand, ["install", "--omit=dev", "--no-audit", "--no-fund", "--package-lock=false"], {
        cwd: PROJECT_ROOT,
        env: process.env,
        stdio: "inherit",
    });
...
async function setupReviewer(existing) {
    if (!stdin.isTTY) {
        throw new Error("No reviewer is configured and interactive setup is unavailable. Set provider environment variables before running the command.");
    }
    ...
    if (provider === "1") {
        ...
        next.OPENAI_COMPATIBLE_MODELS = await promptRequired("Reviewer model ID(s), comma-separated");
