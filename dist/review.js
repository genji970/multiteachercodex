...
async function runOneReviewer(reviewer, prompt, timeoutMs) {
...
export async function runReviews(reviewers, prompt, timeoutMs) {
    return Promise.all(reviewers.map((reviewer) => runOneReviewer(reviewer, prompt, timeoutMs)));
}
export function formatReviewsForRevision(results) {
    return results
        .map((result, index) => {
        const label = `Reviewer ${index + 1}`;
        if (!result.ok)
            return `${label}: review failed (${result.error ?? "unknown error"})`;
        if (!result.review) {
            return `${label}:\n${result.raw_text}`;
        }
        const issues = result.review.issues.length
            ? result.review.issues
                .map((issue, issueIndex) => `${issueIndex + 1}. [${issue.severity}/${issue.category}]\n` +
