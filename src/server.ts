...
function logHeader(title: string, reviewId?: string): void {
  console.log(`\n${line()}`);
...
function formatReviewerResult(result: ReviewerResult, index: number): string {
  const lines = [
    `Reviewer ${index + 1}`,
    `id: ${result.reviewer}`,
  ...
  if (result.review) {
    ...
    for (const [issueIndex, issue] of result.review.issues.entries()) {
      ...
      lines.push(
        `  required_content: ${issue.required_content.length ? issue.required_content.join(" | ") : "(none)"}`,
      );
    }
  }
...
function parseReviewRequest(value: unknown): ReviewRequest {
  ...
  const originalQuestion =
    typeof input.original_question === "string"
      ? input.original_question.trim()
      : "";
  const chatgptAnswer =
    typeof input.chatgpt_answer === "string"
      ? input.chatgpt_answer.trim()
      : "";
  const focus = typeof input.focus === "string" ? input.focus.trim() : "";

  if (!originalQuestion) throw new Error("original_question is required.");
  if (!chatgptAnswer) throw new Error("chatgpt_answer is required.");
...
function parseBrowserEvent(value: unknown): BrowserEventRequest {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("Expected a JSON object.");
  }
  ...
  return {
    event: event as BrowserEventRequest["event"],
...
function logBrowserEvent(event: BrowserEventRequest): void {
  if (event.event === "revision_submitted") {
    ...
    logSection(
      "DELIVERED REVISION INSTRUCTION",
      event.content || "The extension confirmed submission, but no instruction text was supplied.",
    );
    return;
  }
...
async function createReviewPayload(
  input: ReviewRequest,
  reviewId = randomUUID(),
  ...
  if (reviewers.length === 0) {
    throw new Error(
      "No external reviewer is configured. Run npx -y . --setup and configure at least one provider.",
    );
  }
  ...
  if (input.focus) logSection("ADDITIONAL REVIEW FOCUS", input.focus);
  logSection(
    "REVIEWERS",
    reviewers.map((reviewer) => `${reviewer.provider}:${reviewer.model}`).join("\n"),
  );
  ...
  const issueCount = successful.reduce(
    (sum, result) => sum + (result.review?.issues.length ?? 0),
    0,
  );
...
function createReviewerServer(): McpServer {
  ...
  server.registerTool(
    "review_chatgpt_answer",
    {
      ...
      description:
        "Mandatory final-quality gate for this app. After internally drafting an answer, send the exact user request and the complete hidden draft to independent frontier-model reviewers. Use the returned revision_instruction to revise the draft before showing any answer to the user.",
      inputSchema: {
        original_question: z
          .string()
          .min(1)
    ...
    async ({ original_question, chatgpt_answer, focus }) => {
      try {
          ...
          content: [
            {
              type: "text",
              text:
                "The hidden draft has been independently reviewed. Evaluate the feedback, revise the draft, and return only the complete final answer.\n\n" +
        ...
        console.error(
          `[${timestamp()}] MCP review failed: ${error instanceof Error ? error.message : String(error)}`,
        );
        return {
          isError: true,
          content: [
