...
function setStatus(text: string, tone: "working" | "error" | "done" = "working"): void {
...
function clearStatus(delayMs = 0): void {
  window.setTimeout(() => {
    document.getElementById("multiteachercodex-status")?.classList.remove("mtc-visible");
  }, delayMs);
}
...
function findNextTurn(turns: Turn[], node: HTMLElement, role: Turn["role"]): Turn | null {
...
function hidePersistedInternalTurns(turns: Turn[]): void {
  for (let index = 0; index < turns.length; index += 1) {
    ...
    if (turn?.role !== "user" || !turn.text.startsWith(INTERNAL_MARKER)) continue;
    hideTurn(turn);
    ...
    for (let previous = previousAssistantIndex - 1; previous >= 0; previous -= 1) {
      const previousTurn = turns[previous];
      if (previousTurn?.role === "user" && !previousTurn.text.startsWith(INTERNAL_MARKER)) {
...
async function reviewAndRevise(question: string, draft: string, token: number): Promise<void> {
  phase = "reviewing";
  ...
  if (!response.ok || typeof instruction !== "string" || !instruction.trim()) {
    ...
    setStatus(`MultiTeacherCodex failed: ${response.error ?? "invalid review response"}`, "error");
    clearStatus(7000);
    resetCycle();
    return;
  }
  ...
  try {
    ...
    showTurn(activeDraftNode);
    setStatus(
      `MultiTeacherCodex failed: ${error instanceof Error ? error.message : String(error)}`,
      "error",
    );
    clearStatus(7000);
    resetCycle();
...
function reconcile(): void {
  ...
  if ((phase === "sending-revision" || phase === "awaiting-final") && activeDraftNode) {
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  ...
  if (changes.serverUrl && typeof changes.serverUrl.newValue === "string") {
    settings.serverUrl = changes.serverUrl.newValue.trim().replace(/\/+$/u, "");
  }
  operationToken += 1;
  resetCycle();
  scheduleReconcile();
